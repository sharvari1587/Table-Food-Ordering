...
const getAllOrders = async (req, res) => {
  try {
    const orders = await Food.find().sort({ createdAt: -1 }); // Retrieve orders sorted by creation date (newest first)
    res.json(orders);
...
const togglePaymentStatus = async (req, res) => {
  ...
  try {
    const updatedOrder = await Food.findByIdAndUpdate(orderId, { paid }, { new: true });
